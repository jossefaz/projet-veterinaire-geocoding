...
def generate_adress_list() -> list:
    ...
    for filename in os.listdir(RESOURCE_DIR):
        if filename.endswith(".txt"):
            ...
            with open(fichier, 'r', encoding='utf-8-sig') as address_txt_file:
                address_list_local = []
                phones = [None,None]
                for line in address_txt_file:
                    if line == "\n":
                        try:
                            nom, prenom = generate_name(address_list_local[0].strip())
                            clinique = ' '.join(address_list_local[1:-2]).strip()
                            address = address_list_local[-2].strip()
                            zipcode, town_name = generate_town(address_list_local[-1].strip())
...
def generate_csv(address_list: list):
    ...
    with open(out_csv_path, 'w', newline='') as file:
        ...
        writer.writerow(["Nom", "Prenom", "nom_clinique", "Telephone_1", "Telephone_2", "Adresse", "ville", "code_postal","lat", "lon"])
        count = 0
        total = len(address_list)
        for line in address_list:
            lat, lon = get_lat_lon(line[5], line[6], line[7])
            line.append(lat)
            line.append(lon)
